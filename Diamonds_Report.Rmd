...
rf_fit_tune_fine %>%
  ...
  ylab("Mean RMSE") +
  ggtitle("10-fold Cross Validation Mean RMSE Estimates/nfor RF Tuning Parameters mtry and min_n",
          subtitle = "Error bars represent +/- 1 standard error. Points are dodged to avoid overlapping.")
```
...
This calculation may be more useful for both buyers and sellers of round cut diamonds as there may be an acceptable range for the predictions 

```{r perc diff dist, out.width='125%'}
preds_price <- preds_price %>%
...
preds_price %>%
  ...
  xlab("Percent Error") +
  ylab("Density") +
  ggtitle("Distribution of the Percent Error\nBetween the Predicted and Observed Prices")
```
...
split_df %>%
  ...
  column_spec(column = 2:10, width = "3em")
```
...
perc_split_df <- data.frame("price_group" = split_df$price_group,
                            percerr_1 <- c(preds_price %>% filter(price < 5000 & percent_err < 1) %>%
                                              count()/nrow(preds_price %>% filter(price < 5000)) %>% unlist(),
                                            preds_price %>% filter(price >= 5000 & percent_err < 1 & price < 10000) %>%
                                              count()/nrow(preds_price %>% filter(price >= 5000 & price < 10000)) %>% unlist(),
                                            preds_price %>% filter(price >= 10000 & percent_err < 1) %>%
                                              count()/nrow(preds_price %>% filter(price >= 10000)) %>% unlist()) %>% unlist(),
                            percerr_5 <- c(preds_price %>% filter(price < 5000 & percent_err < 5) %>%
                                              count()/nrow(preds_price %>% filter(price < 5000)) %>% unlist(),
                                            preds_price %>% filter(price >= 5000 & percent_err < 5 & price < 10000) %>%
                                              count()/nrow(preds_price %>% filter(price >= 5000 & price < 10000)) %>% unlist(),
                                            preds_price %>% filter(price >= 10000 & percent_err < 5) %>%
                                              count()/nrow(preds_price %>% filter(price >= 10000)) %>% unlist()) %>% unlist(),
                            percerr_10 <- c(preds_price %>% filter(price < 5000 & percent_err < 10) %>%
                                              count()/nrow(preds_price %>% filter(price < 5000)) %>% unlist(),
                                            preds_price %>% filter(price >= 5000 & percent_err < 10 & price < 10000) %>%
                                              count()/nrow(preds_price %>% filter(price >= 5000 & price < 10000)) %>% unlist(),
                                            preds_price %>% filter(price >= 10000 & percent_err < 10) %>%
                                              count()/nrow(preds_price %>% filter(price >= 10000)) %>% unlist()) %>% unlist(),
                            percerr_15 <- c(preds_price %>% filter(price < 5000 & percent_err < 15) %>%
                                              count()/nrow(preds_price %>% filter(price < 5000)) %>% unlist(),
                                            preds_price %>% filter(price >= 5000 & percent_err < 15 & price < 10000) %>%
                                              count()/nrow(preds_price %>% filter(price >= 5000 & price < 10000)) %>% unlist(),
                                            preds_price %>% filter(price >= 10000 & percent_err < 15) %>%
                                              count()/nrow(preds_price %>% filter(price >= 10000)) %>% unlist()) %>% unlist(),
                            percerr_20 <- c(preds_price %>% filter(price < 5000 & percent_err < 20) %>%
                                              count()/nrow(preds_price %>% filter(price < 5000)) %>% unlist(),
                                            preds_price %>% filter(price >= 5000 & percent_err < 20 & price < 10000) %>%
                                              count()/nrow(preds_price %>% filter(price >= 5000 & price < 10000)) %>% unlist(),
                                            preds_price %>% filter(price >= 10000 & percent_err < 20) %>%
                                              count()/nrow(preds_price %>% filter(price >= 10000)) %>% unlist()) %>% unlist())
colnames(perc_split_df) <- c("price_group", "percerr_1", "percerr_5", "percerr_10", "percerr_15", "percerr_20")
```
...
perc_split_df %>%
  ...
  kable(format = "latex", align = "c",
        col.names = c("Price Group", "%error < 1%", "%error < 5%", "%error < 10%", "%error < 20%"))
```
