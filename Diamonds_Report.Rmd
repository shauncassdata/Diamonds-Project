...
```{r scatterplot of numerical vars train data, fig.cap="Bivariate relationships of numerical variables in training data.", out.width='75%', dev="jpeg"}
train_data %>%
  select(price, carat, depth, table, x, y, z) %>%
  drop_na() %>%
...
the_best %>%
  ...
  ggtitle("Seven Best Models Selected For Each Model Type Using 10-Fold CV",
          subtitle = "Error bars are +/- 1 standard error.") +
  theme_classic()  +
...
preds_price %>%
  ...
  ggtitle("Predicted Price vs. Observed Price") +
  scale_color_manual(name = "Ratio", values = c("red" = "red"), labels = "1:1") +
  scale_x_continuous(labels = scales::label_dollar()) +
  scale_y_continuous(labels = scales::label_dollar()) +
...
To further understand how well the model predicted the price of the diamonds, the distribution of the absolute difference between the predicted and observed prices was examined (figure ?). It was found that this distribution had, rounded to the nearest dollar, a sample standard deviation of \$445, a sample mean of $257 with a corresponding standard deviation of \$4, a sample median of \$93, and a sample 75th percentile (the point at which 75% of the data is at or smaller than this value) of \$286. This further cemented the notion that a particular subset of the predictions fared much worse than others. 
...
preds_price %>%
  ...
  scale_x_continuous(labels = scales::label_dollar()) +
  theme_minimal() +
  ggtitle("Distribution of the Absolute Difference\nBetween the Predicted and Observed Prices")
```
In total, there were only 6 predictions that exactly matched the price of the observed diamond. Although the absolute difference 


```{r}
preds_price <- preds_price %>%
  mutate(percent_diff = ((.pred - price)/price)*100)
...
preds_price %>%
  ...
  geom_vline(aes(xintercept = quantile(percent_diff, 0.25), color = "25th Percentile"), linetype = "dashed") +
  geom_vline(aes(xintercept = quantile(percent_diff, 0.75), color = "75th Percentile"), linetype = "twodash") +
  xlab("Percent Difference") +
  ylab("Density") +
  ggtitle("Distribution of the Percent Difference\nBetween the Predicted and Observed Prices")
```
...
perc_split_df <- data.frame("price_group" = split_df$price_group,
                            percdiff_1 <- c(preds_price %>% filter(price < 5000 & percent_diff < 1) %>%
                                              count()/nrow(preds_price %>% filter(price < 5000)) %>% unlist(),
                                            preds_price %>% filter(price >= 5000 & percent_diff < 1 & price < 10000) %>%
                                              count()/nrow(preds_price %>% filter(price >= 5000 & price < 10000)) %>% unlist(),
                                            preds_price %>% filter(price >= 10000 & percent_diff < 1) %>%
                                              count()/nrow(preds_price %>% filter(price >= 10000)) %>% unlist()) %>% unlist(),
                            percdiff_5 <- c(preds_price %>% filter(price < 5000 & percent_diff < 5) %>%
                                              count()/nrow(preds_price %>% filter(price < 5000)) %>% unlist(),
                                            preds_price %>% filter(price >= 5000 & percent_diff < 5 & price < 10000) %>%
                                              count()/nrow(preds_price %>% filter(price >= 5000 & price < 10000)) %>% unlist(),
                                            preds_price %>% filter(price >= 10000 & percent_diff < 5) %>%
                                              count()/nrow(preds_price %>% filter(price >= 10000)) %>% unlist()) %>% unlist(),
                            percdiff_10 <- c(preds_price %>% filter(price < 5000 & percent_diff < 10) %>%
                                              count()/nrow(preds_price %>% filter(price < 5000)) %>% unlist(),
                                            preds_price %>% filter(price >= 5000 & percent_diff < 10 & price < 10000) %>%
                                              count()/nrow(preds_price %>% filter(price >= 5000 & price < 10000)) %>% unlist(),
                                            preds_price %>% filter(price >= 10000 & percent_diff < 10) %>%
                                              count()/nrow(preds_price %>% filter(price >= 10000)) %>% unlist()) %>% unlist(),
                            percdiff_15 <- c(preds_price %>% filter(price < 5000 & percent_diff < 15) %>%
                                              count()/nrow(preds_price %>% filter(price < 5000)) %>% unlist(),
                                            preds_price %>% filter(price >= 5000 & percent_diff < 15 & price < 10000) %>%
                                              count()/nrow(preds_price %>% filter(price >= 5000 & price < 10000)) %>% unlist(),
                                            preds_price %>% filter(price >= 10000 & percent_diff < 15) %>%
                                              count()/nrow(preds_price %>% filter(price >= 10000)) %>% unlist()) %>% unlist(),
                            percdiff_20 <- c(preds_price %>% filter(price < 5000 & percent_diff < 20) %>%
                                              count()/nrow(preds_price %>% filter(price < 5000)) %>% unlist(),
                                            preds_price %>% filter(price >= 5000 & percent_diff < 20 & price < 10000) %>%
                                              count()/nrow(preds_price %>% filter(price >= 5000 & price < 10000)) %>% unlist(),
                                            preds_price %>% filter(price >= 10000 & percent_diff < 20) %>%
                                              count()/nrow(preds_price %>% filter(price >= 10000)) %>% unlist()) %>% unlist())
colnames(perc_split_df) <- c("price_group", "percdiff_1", "percdiff_5", "percdiff_10", "percdiff_15", "percdiff_20")
```
