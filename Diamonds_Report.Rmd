...
To further understand how well the model predicted the price of the diamonds, the distribution of the absolute difference between the predicted and observed prices was examined (figure ?). It was found that this distribution had, rounded to the nearest dollar, a sample standard deviation of \$445, a sample mean of $257 with a corresponding standard deviation of \$4, a sample median of \$93, and a sample 75th quantile of \$286. This further cemented the notion that a relatively small proportion of the predictions fared much worse than others. 
...
```
In total, there were only 6 predictions that exactly matched the price of the observed diamond. 


```{r}
preds_price %>% mutate(percent_diff = ((.pred - price)/price)*100) %>%
...
```{r}
split_df %>%
  mutate(across(2:9, ~round(.x*100, 1))) %>%
  mutate(across(2:9, ~paste0(as.character(.x), "%"))) %>%
  kable(format = "latex", align = "c")
```
